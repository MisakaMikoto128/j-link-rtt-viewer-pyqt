...
为后续维护积累的实际踩坑经验。每条都带 **现象 / 原因 / 处理** 三段。索引见 `CLAUDE.md`。
...
参考：`src/core/jlink_worker.py` 顶部常量、`src/ui/settings_page.py` `_RESET_MODE_LABELS`。
...
参考：`src/core/jlink_worker.py` `_on_connect` / `_do_connect` / `_reset_with_reconnect`。
...
参考：`src/core/jlink_worker.py` `_on_enumerate_devices` / `_do_connect` / `_reconnect_tick`、`src/ui/rtt_monitor_page.py` `_on_devices_enumerated` / `_on_connect_clicked`。
...
参考：`src/core/pack_service.py` `get_pack_cache`。
...
**处理**（Step 1-5，commit `0ae7465` / `9ef221d` / `42e8351` / `7b17f02`）：
