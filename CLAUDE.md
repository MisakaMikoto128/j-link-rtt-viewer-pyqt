...
## UI 模块拆分（rtt_monitor_page.py 3083 -> 2396）

**背景**：`rtt_monitor_page.py` 曾 3083 行，太大难导航。按职责拆出低耦合部分，主类状态机留主文件。

**处理**（Step 1-2 + Step 5，commit `0ae7465` / `9ef221d` / `42e8351`）：
...
**未做（高风险，下一轮）**：`_SendBar` / `_DisplayArea` / `_CommandPanel` 拆分。这些涉及核心状态（`_is_connected` / `_channel_buffers` / `_all_rtt_buffer` / `_view_channel` / `_programmatic_scroll` / `_send_total_bytes`）+ worker 信号 + scroll guard，且和主类 `_on_state_changed`/`_on_channel_changed` 深度纠缠，CLAUDE.md 警告的 UI↔worker 信号协定 + scroll/timer 踩坑都会重新踩。建议先补集成测试覆盖 UI 交互（连接/断开/发送/显示/通道切换）再拆。
