...
class RTTMonitorPage(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self._floating_panel = FloatingPanel(self)
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        self._lbl_iface.setFixedHeight(_CTRL_H)
        ...
        self.cb_iface.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        self.cb_iface.setMinimumWidth(50)
        row_iface.addWidget(self.cb_iface, 1)
        self._lbl_speed = BodyLabel(self.tr("速度"))
        self._lbl_speed.setFixedHeight(_CTRL_H)
        row_iface.addWidget(self._lbl_speed)
        self.cb_speed = ComboBox(inner)
        self.cb_speed.setFixedHeight(_CTRL_H)
        self.cb_speed.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
        self.cb_speed.setMinimumWidth(50)
        ...
        row_iface.addWidget(self.cb_speed, 1)
    ...
    def resizeEvent(self, event: QResizeEvent) -> None:
        ...
        # 悬浮卡片跟随窗口尺寸重定位（仅可见时）
        self._floating_panel.reposition(self.height())
    ...
    def _set_config_panel_visible(self, visible: bool) -> None:
        ...
        if visible:
            ...
            if self._config_panel.parent() is not self._floating_panel.card_widget():
                pass  # 已在布局中，无需操作
            else:
                self._floating_panel.content_layout().removeWidget(self._config_panel)
                self._config_panel.setParent(self)
            self._main_split.insertWidget(0, self._config_panel)
            # 隐藏悬浮卡片 + 复位 toggle 按钮（不触发 toggled 信号）
            self._floating_panel.stop_animations()
            ...
            self._config_panel.setParent(self._floating_panel.card_widget())
            self._floating_panel.content_layout().addWidget(self._config_panel)
    ...
    def _on_panel_toggle_toggled(self, checked: bool) -> None:
        ...
        if checked:
            self._floating_panel.show_card(self.height())
        else:
            self._floating_panel.hide_card()
    ...
    def on_shortcut_find(self) -> None:
        self._search_handler.on_shortcut_find()

    def on_shortcut_replace(self) -> None:
        self._search_handler.on_shortcut_replace()
