"""设置页：外观 + RTT 行为 + 背景图 + 重置按钮行为。

分组：
- 外观：主题（浅/深/跟随系统）/ 主题色 / 界面字体 family+字号 / RTT 字体 /
  内存页字号 / 语言
- RTT 行为：最大显示行数 / 轮询间隔 / 解码编码 / 自动断帧 / 发送换行符 / 会话标记色
- 重置按钮行为：normal / auto_reconnect
- 背景：图片路径 / 透明度 / 填充方式
- 其他：保持屏幕常亮 / 日志目录

所有改动经 ConfigService.set 持久化（200ms 节流），信号驱动 UI 热应用。
"""
