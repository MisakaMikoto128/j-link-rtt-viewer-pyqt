...
需要 **Python 3.10+**（实测 3.13 启动最快，详见 [Python 版本对比](docs/python-version-comparison.md)）。
...
git clone https://github.com/MisakaMikoto128/j-link-rtt-viewer-pyqt.git
cd j-link-rtt-viewer-pyqt
uv venv --python 3.13           # 建到 .venv\
uv pip install -r requirements.txt
uv run python src/main.py
# 或双击 start.bat（自动识别 venv\ 或 .venv\）
```

> `start.bat` 会按 `venv\` -> `.venv\` 顺序找虚拟环境；两个都没有时打印创建提示并退出。pip 装到 `venv\`、uv 装到 `.venv\` 都能被识别。
